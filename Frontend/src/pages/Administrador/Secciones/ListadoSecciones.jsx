...
const SeccionesPage = () => {
  ...
  const getSemesterStatus = (semesterId) => {
    const semester = semesters.find(sem => sem._id === semesterId);
    return semester ? semester.status : "Semestre no encontrado";
  };
  ...
  const openDeleteModal = (section) => {
    const semesterStatus = getSemesterStatus(section.semesterId);
    if (semesterStatus === 'en curso' || semesterStatus === 'cerrado') {
      alert('No se puede eliminar una sección de un semestre en curso o cerrado.');
      return;
    }
  ...
  const columns = [
    ...
    {
      name: "Carrera",
      selector: (row) => getCareerNameFromSubject(row.subjectId)
    },
    {
      name: "Semestre",
  ...
  const filterFunction = (data, query) => {
    return data.filter(section => 
      ...
      getCareerNameFromSubject(section.subjectId).toLowerCase().includes(query.toLowerCase()) ||
